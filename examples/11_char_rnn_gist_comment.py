...
def create_rnn(seq, hidden_size=HIDDEN_SIZE):
    ## input seq is [batch size, seqlen, alphasize]
    cell = tf.nn.rnn_cell.GRUCell(hidden_size)
    ## in state is [batch size, hidden size], in this example is only one layer
...
def create_model(seq, temp, vocab, hidden=HIDDEN_SIZE):
    ## [batch size, seqlen, alphasize], alphasize is the one hot coding
...
def training(vocab, seq, loss, optimizer, global_step, temp, sample, in_state, out_state):
    ...
    with tf.Session() as sess:
        ...
        ## in fact, it does not get a batch, only 1 sequence
