...
def conv(input, name, filter_dims, stride_dims, padding='SAME',
    ...
    with tf.variable_scope(name) as scope:
        ...
        kernel = tf.get_variable('kernel', [filter_h, filter_w, num_channels_in, num_channels_out], initializer=tf.truncated_normal_initializer())
        # Create bias variable
        biases = tf.get_variable('biases', [num_channels_out], initializer=tf.random_normal_initializer())
        ...
        if non_linear_fn:
            output = non_linear_fn(output, name=scope.name)
        return output
...
def deconv(input, name, filter_dims, stride_dims, padding='SAME',
    ...
    with tf.variable_scope(name) as scope:
        ...
        kernel = tf.get_variable('kernel', [filter_h, filter_w, num_channels_out, num_channels_in], initializer=tf.truncated_normal_initializer())
        # Create bias variable
        biases = tf.get_variable('biases', [num_channels_out], initializer=tf.random_normal_initializer())
        # Define the deconv flow graph
        conv = tf.nn.conv2d_transpose(input, kernel, output_dims, strides=[1, stride_h, stride_w, 1], padding=padding)
...
def fc(input, name, out_dim, non_linear_fn=tf.nn.relu):
    ...
    with tf.variable_scope(name) as scope:
        ...
        w = tf.get_variable('weigths', [in_dim, out_dim], initializer=tf.truncated_normal_initializer())
        # Create bias variable
        b = tf.get_variable('biases', [out_dim], initializer=tf.random_normal_initializer())
        # Define FC flow graph
        output = tf.matmul(flat_input, w) + b
        # Apply non-linearity (if asked) and return output
        if non_linear_fn:
            output = non_linear_fn(output, name=scope.name)
        return output
